...
* Asegúrese de que la conexión en caliente de la CPU y la conexión en caliente de la memoria están activadas durante el despliegue. Puede habilitarlos en el menú del cliente de vCenter si no se habilitaron durante la implementación. Siga estas instrucciones para habilitarlos desde el cliente vCenter:
+
** Vaya a Herramientas de ONTAP para la máquina virtual VMware vSphere en *Inventories* y seleccione *Editar configuración*.
