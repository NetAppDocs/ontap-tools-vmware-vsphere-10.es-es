...
Añada el back-end de almacenamiento a un clúster de ONTAP. Para configuraciones de multi-tenancy en las que vCenter actúa como inquilino con la SVM asociada, utilice ONTAP tools Manager para agregar el clúster. Después de añadir el back-end de almacenamiento al clúster global, debe asociar el clúster con los inquilinos de vCenter deseados. El inquilino de vCenter debe incorporar las máquinas virtuales de almacenamiento (SVM) que desee. Esto permite que un usuario de SVM aprovisione un almacén de datos vVols. Puede añadir almacenamiento en vCenter mediante la SVM.

Para añadir un back-end de almacenamiento a un clúster local, utilice el complemento de herramientas de ONTAP en vCenter Server.

link:../concepts/ontap-tools-concepts-terms.html["Conceptos y términos clave"] proporciona más información sobre los términos usados en esta sección.
