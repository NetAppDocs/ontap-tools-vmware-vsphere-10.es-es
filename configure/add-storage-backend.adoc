...
summary: Agrega y administra backends de almacenamiento localmente para una instancia de vCenter o globalmente para el descubrimiento a nivel de clúster en las herramientas de ONTAP. 
---
= Agrega un backend de almacenamiento a ONTAP tools
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Utiliza ONTAP tools for VMware vSphere para añadir y gestionar backends de almacenamiento para tus hosts ESXi, incluyendo la incorporación de clústeres o SVM, la habilitación de la compatibilidad con MetroCluster y la validación de certificados para una conectividad segura. Puedes configurar backends de almacenamiento usando ONTAP tools Manager o el vSphere Client, supervisar el estado de los certificados y volver a descubrir manualmente los recursos después de cambios en el clúster.

Para agregar un backend de almacenamiento localmente, use las credenciales del clúster o SVM en la interfaz de herramientas de ONTAP .  Los backends de almacenamiento local solo están disponibles para el vCenter Server seleccionado. Las herramientas de ONTAP asignan SVM al vCenter Server para la administración del almacén de datos vVols o VMFS.  Para los almacenes de datos VMFS y los flujos de trabajo de SRA, puede usar credenciales de SVM sin asignar un clúster globalmente.

Para añadir un backend de almacenamiento global, utiliza las credenciales del clúster ONTAP en ONTAP tools Manager. Los backends de almacenamiento global permiten que los flujos de trabajo de descubrimiento identifiquen los recursos del clúster necesarios para la gestión de vVols. En entornos multitenant, puedes añadir un usuario SVM de forma local para gestionar los datastores de vVols.

Consulta el artículo de KB: https://kb.netapp.com/data-mgmt/OTV/VSC_Kbs/OTV_10_What_is_storage_backend_global_and_local_scope["OTV 10: ¿Qué es el ámbito global y local del backend de almacenamiento?"^] para obtener más información sobre los backends de almacenamiento global y local.

Si la compatibilidad con NetApp MetroCluster está habilitada en ONTAP, incorpora tanto el clúster de origen como el de destino como backends de almacenamiento locales o globales.
...
--
====
ONTAP tools muestra el backend de almacenamiento recién añadido en la página *Storage backends*. Si un certificado caduca en 30 días o menos, ONTAP tools muestra una advertencia en la columna de fecha de caducidad del certificado. Después de la caducidad, ONTAP tools marca el backend de almacenamiento como desconocido porque no puede conectarse al sistema de almacenamiento.

.Información relacionada
https://kb.netapp.com/data-mgmt/OTV/VSC_Kbs/OTV_10_What_is_storage_backend_global_and_local_scope["OTV 10: ¿Qué es el ámbito global y local del backend de almacenamiento?"^]
