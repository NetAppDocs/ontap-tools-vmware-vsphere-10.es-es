...
summary: Configura los roles y privilegios de los usuarios para los backends de almacenamiento con el archivo JSON de ONTAP tools y ONTAP System Manager. 
---
...
[role="lead"]
Configura los roles y privilegios de usuario de ONTAP para los backends de almacenamiento usando ONTAP tools for VMware vSphere y ONTAP System Manager. Puedes asignar roles con los archivos JSON proporcionados, crear usuarios y roles manualmente o aplicar los privilegios mínimos necesarios para las cuentas que no sean de administrador.

.Antes de empezar
* Descarga el archivo ONTAP Privileges de ONTAP tools for VMware vSphere en _\https://<ONTAPtoolsIP>:8443/virtualization/user-privileges/users_roles.zip_. El archivo zip contiene dos archivos JSON. Utiliza el archivo JSON específico para ASA r2 cuando configures un sistema ASA r2.
+
...
.. Selecciona la capacidad del producto como *VSC, VASA Provider y SRA* en la lista desplegable.
+
...
En esta página se enumeran los privilegios mínimos necesarios para un usuario del clúster sin derechos de administrador, de ámbito global y sin archivo JSON. Si un clúster tiene ámbito local, usa el archivo JSON para crear usuarios porque ONTAP tools for VMware vSphere necesita más que solo los privilegios de lectura para el aprovisionamiento en ONTAP.
