...
summary: Utiliza los códigos de error de actualización de ONTAP tools para identificar los flujos de trabajo fallidos y solucionar los problemas de actualización en VMware vSphere. 
---
...
| 068 | Ha fallado la reversión de los paquetes del sistema operativo Debian | Utilice la recuperación basada en RPO o Snapshot cero y vuelva a intentar la actualización. 
...
| 074 | El montaje ISO ha fallado | Comprueba el archivo /var/log/otvng/upgrade-run.log y vuelve a intentar la actualización. 
...
| 084 | Fallo al volver a cambiar JobManager a producción | Sigue estos pasos para recuperar o completar la actualización. 1. Activa el shell de diagnóstico 2. Ejecuta el comando: _sudo perl /home/maint/scripts/post-deploy-upgrade.pl --postupgrade_ 3. Revisa los registros en /var/log/otvng/post-deploy-upgrade.log 


| 087 | Fallo en los pasos posteriores a la actualización. | Sigue estos pasos para recuperar o completar la actualización. 1. Activa el shell de diagnóstico 2. Ejecuta el comando _sudo perl /home/maint/scripts/post-deploy-upgrade.pl --postupgrade_ 3. Comprueba los registros en /var/log/otvng/post-deploy-upgrade.log 
...
| 094 | La configuración de NTP falló y el sistema volvió a NTPsec  a| 
Si la actualización falla debido a un error en la instalación de Chrony:

. Comprueba si Chrony está instalado:
+
[listing]
----
systemctl status chrony
----
. Si tienes instalado Chrony y el problema radica en los servidores NTP que has indicado, comprueba si se puede acceder a dichos servidores:
+
[listing]
----
...
----
+
Si se puede acceder a los servidores, el problema no está relacionado con la accesibilidad de los mismos. Si un servidor es realmente inaccesible, elimínalo de ONTAP tools Manager y proporciona en su lugar un servidor NTP accesible.

. Si la actualización falla debido a cualquier otro problema durante la actualización de NTP, comprueba los siguientes registros:
+
[listing]
----
/var/log/otvng/ansible-perl-errors.log
/var/log/otvng/ntp_configure.log
/var/log/ansible-perl-errors.log
----
...
| 098 | Error al copiar el secreto dockercred de ntv-system al espacio de nombres rook-ceph | Compruebe los registros de actualización y vuelva a intentar la actualización. 
...
| 090 | La validación del certificado para los backends de almacenamiento y vCenter falló | Comprueba los registros de actualización y el archivo de registro en /var/log/otvng/cert_validation_error.log y vuelve a intentar la actualización. 
