...
NOTE: El adaptador de replicación de almacenamiento (SRA) admite distribución ramificada con una relación síncrona del tipo conmutación por error automática dúplex y relación asíncrona SnapMirror en el grupo de consistencia. Sin embargo, no se admite la distribución ramificada con dos SnapMirror asíncrono en el grupo de consistencia o los SnapMirror ramificados en volumen.
