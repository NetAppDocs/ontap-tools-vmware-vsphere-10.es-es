...
summary: Usa los códigos de error de implementación de ONTAP tools para diagnosticar y resolver fallos de implementación, reinicio y recuperación. 
---
= Códigos de error en la implementación de ONTAP tools
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Si experimentas problemas de implementación, revisa los registros y los códigos de error para diagnosticar y resolver problemas. A partir de ONTAP tools for VMware vSphere 10.6, los paquetes de registros recopilados de los pods incluyen registros de MongoDB, RabbitMQ y Vault, junto con el estado y las descripciones de todos los pods. ONTAP tools 10.6 también añade registros de auditoría para mejorar la trazabilidad durante la resolución de problemas. Estos se proporcionan además de los registros de servicio existentes de ONTAP tools, mejorando la capacidad de soporte y la resolución de problemas.
...
Puedes recopilar los archivos de registro de ONTAP tools for VMware vSphere desde las opciones disponibles en la interfaz de usuario de ONTAP tools Manager. El soporte técnico podría pedirte que recopiles los archivos de registro para ayudar a solucionar un problema.


NOTE: La generación de registros desde el administrador de ONTAP tools incluye todos los registros de todas las instancias de vCenter Server. La generación de registros desde la interfaz de usuario del cliente vCenter está limitada al vCenter Server seleccionado.
...
Todos los registros de errores se guardan en el archivo ansible-perl-errors.log, ubicado en el directorio /var/log/otvng, para facilitar el seguimiento y la resolución de problemas. Este archivo de registro contiene el código de error y la tarea de Ansible que falló.
...
| 050 | Error en la generación de claves SSH | Reiniciar la máquina virtual primaria (VM). 
...
| 055 | Fallo al desplegar el registro | Si el pod de registro está presente, espera a que el pod esté listo y luego reinicia la máquina virtual principal o vuelve a desplegar. 


| 059 | El despliegue de KubeVip ha fallado | Asegúrate de que la dirección IP virtual para el plano de control de Kubernetes y la dirección IP de ONTAP tools proporcionadas durante el despliegue pertenezcan a la misma VLAN y sean direcciones IP libres. Reinicia si todos los puntos anteriores son correctos. Si no, vuelve a desplegar. 


| 060 | Fallo en el despliegue del operador | Reinicie 


| 061 | Fallo en el despliegue de servicios | Realiza tareas básicas de depuración de Kubernetes como get pods, get rs, get svc, etc., en el espacio de nombres ntv-system para obtener más detalles y consultar los registros de errores en /var/log/otvng/ansible-perl-errors.log y /var/log/otvng/ansible-run.log y vuelve a implementar. 


| 062 | Se ha producido un error en la implementación de ONTAP TOOLS Services | Consulta los registros de errores en /var/log/otvng/ansible-perl-errors.log para obtener más detalles y vuelve a realizar la implementación. 


| 065 | No se puede acceder a la URL de la página de Swagger | Volver a desplegar 


| 066 | Se ha producido un error en los pasos posteriores al despliegue del certificado de gateway  a| 
Haz lo siguiente para recuperar/completar la actualización:

* Habilita el shell de diagnóstico.
* Ejecuta el comando 'sudo perl /home/maint/scripts/post-deploy-upgrade.pl --postDeploy'.
