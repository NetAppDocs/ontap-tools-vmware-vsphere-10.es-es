...
|===
. En el campo *Seleccione un nombre y carpeta*, introduzca el nombre de la máquina virtual y elija su ubicación. Para habilitar la posibilidad de usar la función HA, seleccione la opción *Personalizar el hardware de esta máquina virtual*, que activará un paso adicional llamado *Personalizar hardware* antes de continuar con la ventana *Listo para completar*.
...
. En la ventana *Personalizar hardware*, habilite las opciones *CPU hot add* y *Memory hot plug* para permitir la funcionalidad HA.
