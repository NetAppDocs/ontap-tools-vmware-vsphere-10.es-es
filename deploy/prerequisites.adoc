...
summary: Consulta las versiones compatibles, los requisitos de tamaño, los requisitos de puertos y los límites de configuración antes de implementar ONTAP tools for VMware vSphere. 
---
...
[role="lead"]
Consulta las versiones compatibles, los requisitos de tamaño, los requisitos de puertos y los límites de configuración antes de implementar ONTAP tools for VMware vSphere.

Implementa las ONTAP tools for VMware vSphere en un entorno compatible con vSphere que incluya hosts ESXi y el dispositivo virtual VMware vCenter Server (vCSA).
...
* *Requisitos de dimensionamiento del sistema host* La tabla siguiente muestra la CPU, la memoria y el espacio en disco recomendados por nodo para cada tamaño de implementación. Para implementaciones de alta disponibilidad (HA), implementa tres nodos.
...
NOTE: El tamaño de implementación grande solo es compatible con la configuración HA.


NOTE: Cuando la copia de seguridad está activada, cada clúster de ONTAP tools necesita 50 GB adicionales de espacio en el datastore donde se implementan las máquinas virtuales. Por lo tanto, non-HA requiere 400 GB y HA requiere 1100 GB de espacio en total.
...
| Hosts ESXi gestionados mediante ONTAP tools | a partir de 7.0.3 


| vCenter instancias de servidor gestionadas por ONTAP tools | A partir de la versión 8.0U3 
...
| Host ESXi usado para implementar la máquina virtual de ONTAP tools | 7.0U3 y 8.0U3 


| vCenter Server utilizado para implementar la máquina virtual de ONTAP tools | 7,0 y 8,0 
|===

NOTE: A partir de ONTAP tools for VMware vSphere 10.4, la versión de hardware de la máquina virtual pasa de 10 a 17.
...
* Puertos entre nodos: Estos puertos permiten la comunicación entre los nodos dentro del clúster de Kubernetes. Son necesarios para operaciones del clúster como el intercambio de datos y la coordinación entre nodos. En las implementaciones de un solo nodo, los puertos entre nodos solo se utilizan dentro del nodo y no requieren acceso externo. Permite el tráfico entre nodos únicamente desde redes de clúster de confianza y bloquea el acceso a internet mediante reglas de cortafuegos.
...
== Límites de configuración para los almacenes de datos vVol

Utiliza la siguiente tabla como guía para configurar ONTAP tools for VMware vSphere.
...
| HA | Pequeño (S) | hasta 24K | 64 


| HA | Medio (M) | hasta 50K | 128 


| HA | Grande (L) | hasta 100K | 256 
|===
...
== Límites de configuración para la detección del backend de almacenamiento

Las herramientas de ONTAP permiten detectar el backend de almacenamiento de hasta 200 SVM y aproximadamente 10,000 LUN.



[[configuration-limits-for-vmfs-and-nfs-datastores]]
== Límites de configuración para los almacenes de datos VMFS y NFS
...
|===
| *vCenter tamaño de la implementación* | *Pequeño* | *Media* 
...
|===
La siguiente tabla muestra el número de instancias de VMware Live Site Recovery y las herramientas ONTAP correspondientes para el tamaño de despliegue de VMware vSphere.

|===


| *Número de instancias de VMware Live Site Recovery* | *Tamaño de la implementación de las herramientas de ONTAP* 
